...
void thread_funct(Graph *graph, fstream * logfile , bool debug, int thread_id, int no_of_threads, int sleep_time){
    ...
    for (long node_id = thread_id; node_id < graph->getTotalNodes() -2; node_id += no_of_threads){
        ...
        if(sleep_time > 0){
            if(debug)
                (*logfile) << "Thread num :: " << thread_id << " is delayed by " << sleep_time << "ms" << endl;
...
int main(int argc, char** argv){
    ...
    for (int thread_id =0;thread_id < num_of_threads; thread_id++){
        ...
        if(sleep_thread_nums.find(thread_id) != sleep_thread_nums.end()){
            thrd_sleep_time = sleep_time;
            if(debug)
                logfile << "Thread num :: " << thread_id << " will be slow" << endl;
        }
        thrds.push_back(std::thread(thread_funct, graph , &logfile , debug, thread_id , num_of_threads , thrd_sleep_time) );
