...
class Graph{
    ...
    long fetchNextNodeAndCheck() {
        ...
        if (node_id >= total_nodes - 2)//last and second last nodes will have only 1 or 0 remaining nodes hence can be ignored
            return -1;
        else
            return node_id;
...
void thread_funct(Graph *graph, fstream * logfile , bool debug, int sleep_time){
    long next_node = graph->fetchNextNodeAndCheck();
    
    //each thread fetches the next available node to calculate the triangle count.
    while(next_node != -1){
        if(debug)
        ...
        graph->triangle_count( next_node, logfile, debug);
        double endTime = chrono::duration_cast < chrono::milliseconds > (chrono::system_clock::now().time_since_epoch()).count();
        if(debug){
            (*logfile) << "Thread id : " << this_thread::get_id() << " Node Id : " <<  next_node << " Time taken : " << endTime - startTime << endl;
            (*logfile) << "Thread id : " << this_thread::get_id() << " Total Traingles : " <<  graph->getNoOfTriangles() << endl;
        }
        if(sleep_time > 0){
            if(debug)
                (*logfile) << "Thread num :: " << this_thread::get_id() << " is delayed by " << sleep_time << "ms" << endl;
...
int main(int argc, char** argv){
	...
	if(argc > 1){
        ...
        sleep_time_str = argv[4];
        sleep_thread_cnt_str = argv[5];
		if(argc ==7 ){
			 if(boost::iequals(argv[6], "debug")){
            	debug = true;
       		 }
		}
	}
    ...
    for (int i =0;i < num_of_threads; i++){
        ...
        thrds.push_back(std::thread(thread_funct, graph , &logfile , debug, thrd_sleep_time) );
