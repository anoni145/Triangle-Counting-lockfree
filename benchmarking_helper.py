...
with open( output_file, 'w+') as f_object:
  ...
  lst = ["", "" ,1] 
  #iterate through thread cnt
  for thread_cnt in term_thread_cnt:
...
with open(script_log_file, 'w+') as log_f_object:
  with open(output_file, 'a+') as f_object:
    ...
    for j in range (len(input_files)) :
      ...
      for thread_cnt in term_thread_cnt :
        print("Termination thread cnt : " + str(thread_cnt),file = log_f_object,flush = True)
        cmd2 = "./algorithms/helper.out " + "./output/logparr_" + date_time_obj +".txt "  + "./datasets/" +input_file + " " + str(threads) + " " + str(thread_cnt) + " " + str(sleep_time) +" " + str(sleep_threads)
        if debug:
          cmd2 += " debug"
        time_taken_par_list = []
