...
total_threads = 16
input_files = ["14.txt"]
parr_program = "tri_count_parr_static_sleep.cpp"
...
seq_iterations = 3
term_thread_cnt = []
sleep_times = [5,10,15,20,25,30,35,40,45,50]
...
with open( output_file, 'w+') as f_object:
    ...
    lst = ["", "" ,1] 
    for i in range(len(threads_lst)):
        lst.append(str(threads_lst[i]) + " sleep threads" )
        lst.append("")
        for j in range(len(sleep_times)-1) :
            lst.extend(["",""])
    ...
    for i in range(len(threads_lst)):
        for j in range(len(sleep_times)) :
            lst.append(sleep_times[j])
            lst.append("Speedup")
    writer_object.writerow(lst)
...
with open(script_log_file, 'w+') as log_f_object:
  with open(output_file, 'a+') as f_object:
    ...
    for j in range (len(input_files)) :
        ...
        cmd1 = "g++ -O3 -o ./algorithms/seq.out ./algorithms/" + seq_program
      ...
      #update sequential iteration
        ...
        cmd1 = "g++ -pthread -O3 -o ./algorithms/parr_sleep.out ./algorithms/" + parr_program
        proc = sp.Popen(cmd1.split())
        proc.wait()

        number_of_triangles_parr = 0
        for sleep_threads in threads_lst:
            print("Sleep threads : " + str(sleep_threads),file = log_f_object,flush = True)

            for sleep_time in sleep_times :
                print("Sleep time : " + str(sleep_time),file = log_f_object,flush = True)
                cmd2 = "./algorithms/parr_sleep.out " + "./output/logparr_" + date_time_obj +".txt "  + "./datasets/" +input_file + " " + str(total_threads) + " "  + str(sleep_time) +" " + str(sleep_threads)
                if debug:
                    cmd2 += " debug"
                time_taken_par_list = []
                ...
                for i in range(parr_iterations):
                    ...
                    if not number_of_triangles_parr:
                        ...
                        print("No o/p for parr for thread cnt : " + str(total_threads) + " and  Iteration :  " + str(i),file = log_f_object,flush = True)
                        continue
                    print("No of triangeles(parr) : " + str(number_of_triangles_parr),file = log_f_object,flush = True)

                    if not time_taken_par:
                        lst.append("")
                        print("No timetake in o/p for parr for thread cnt : " + str(total_threads)+ " and  Iteration : " + str(i),file = log_f_object,flush = True)
                        continue
                ...
                if len(time_taken_par_list)!=0:
                    time_taken_par_list = time_taken_par_list[2:]
                time_taken_par_avg = int(sum(time_taken_par_list)/len(time_taken_par_list))
